...
dir_patch = 'data'
protocol_type = 'fcsProtocolPPI'

key_list = ('epProtocolEOK3',
            'epProtocolEOKOU3',
            'commonInfo',
            'publisherOrg',
            'extPrintFormInfo',
...
def parse_to_db(files):
    ...
    for f in files:
        ...
        for i in parse_xml(f):
            ...
            if 'offer' in i[1].keys() or 'finalPrice' in i[1].keys():
                ...
                ooo = {'INN': '',
                       ...
                       'contactPhone': '',
                       'isIP': ''}
            elif 'rejectReason' in i[0] and 'explanation' in i[1].keys():
                ooo_list.append(ooo.copy())
        ...
        prot_info['regNum'], prot_info['type'] = (org_list['regNum'],
                                                  protocol_type, )
        prot_info['publishDate'] = prot_info.pop(
            'publishDTInEIS')
        try:
            ...
            for p in prot_list:
                if p['offer'] < p['finalPrice']:
                    p['price'] = p.pop('finalPrice')
                else:
                    p['price'] = p.pop('offer')
